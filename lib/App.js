...
class App {
   ...
   */
  constructor() {
      /** @type Load Game Debugger */
      this.debug = new GameDebug()
    try {
      /** @type Load Game Config strings */
      this.config = new GameConfig()
      // Load Game Debugger for the App
      /** @type Load Game controller */
      this.game = new Game('X','Y',this.config)
      /** @type Retrieve start button */
  ...
  init() {
    ...
    try {
      ...
      if (!(this.startListener instanceof StartListener)) {
        this._error(this.startListener)
      }
      ...
      this.moveListener.setListeners()
