...
class App {
    ...
    // button = new Element("start")
    button
    // move = new NodeList("cell")
    move
    startListener = new StartListener()
    moveListeners = new StartListener()
    ...
    constructor() {
        ...
        //Try new props/member assignments, else throw uncaught error
        try {
            debugger;
            this.appConfig = new GameConfig()
            this.newGame = new Game('X', 'Y')
        } catch (e) {
            this._error(e, 0, 10, 'New Props: Not Initaitated/Instance')
        }
        //Try UI for start button, else throw general error
        try {
            debugger;
            this.startButton = document.querySelector('#start')
            this.startListener = new StartListener(this.startButton, this.newGame, 'click')
        } catch (e) {
            this._error(e, 1, 10, 'New: StartListener: On Click Event')
        }
        // Try UI for user moves, else throw general error
        try {
            debugger;
            this.userMoves = document.querySelectorAll('.cell')
            this.moveListeners = new MoveListener(this.userMoves, this.newGame, 'click')
        } catch (e) {
            this._error(e, 1, 10, 'New: MoveListeners Cell Targets: On Click Event')
        }
