...
class Game {
     ...
     * @memberof Game
     */
    onInit(log = 0, locname = `${this.fileName}: onInit`) {
        if (!this.gameRunning) {
     ...
     * @version 0.3.0 INSPECT @date 2023/03/22. See Changelog on this date.
     **/
    isGameOver(log = 0, locname = `${this.fileName}: isGameOver()`) {
        if (this.gameLogic.isAWin()) {
            this.deBug._debug(this.gameLogic.isAWin(), `${locname}: is in a Win state`, log, `${locname}`);
        }
        if (this.gameLogic.isADraw()) {
            this.deBug._debug(this.gameLogic.isADraw(), `${locname}: is in a Draw state`, log, `${locname}`);
        }
