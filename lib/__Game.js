...
import { GameConfig } from "/lib/__GameConfig.js"; // Added 03-14

/**
 * @name Game
 * @summary Game Controller class for the game of TicTacToe
 * @class, @export
 * @property {GameBoard} this.gameBoard Game's board and conditions of play for a starting game board or for reseting the board.
 * @property {GamePieces} this.gamePieces Players' pieces for the game play.
 ...
 */
class Game {
  constructor(_X,_Y, _config) {
    // if (!(this.gameBoards instanceof GameBoard)) {
    // }
    // if (!(this.gamePieces instanceof GamePieces)) {
    // }
    // if (!(this.gameLogic instanceof GameLogic)) {
    // }
    if (!(this.gameConfig instanceof GameConfig)) {
      throw new Error(`X or Y Piece are not string types. ${e.message}: ${_config}`,
        { name: _config, stack: _config, cause: _config
        })
    }
    this.gameBoard = new GameBoard()
    this.gamePieces = new GamePieces(_X,_Y)
    this.gameLogic = new GameLogic(this.gameBoard,this.gamePieces)
    this.gameConfig = _config
    this.debug = new GameDebug()
    // this.Player1 = this.gamePieces.pieces[0]
    this.Player1 = _X
    // this.Player2 = this.gamePieces.pieces[1]
    this.Player2 = _Y
    // Game Start is true
    this.gameStart = true

    this.debug._debug(gameBoards,`Full Stack: ${this.gameBoard}`, 9)
    this.debug._debug(gamePieces,`Full Stack: ${this.gamePieces}`, 9)
    this.debug._debug(gameLogic,`Full Stack: ${this.gameLogic}`, 9)
    this.debug._debug(gameLogic,`Players: 1: ${this.Player1}. 2: ${this.Player2}`, 9)
  }
  ...
  **/
   ...
   */
  makeMove(move) {
    if (this.gameBoard.surface[move] === ''
