...
class Game {
    ...
    * @todo Optionally use this interface to manage the level of difficulty of the game: Just Click, ii) Randomisation of move iii) Minimax Algorithm @date 2033/03/16
    ...
    Player1 = new String(''); /*?+*/
    Player2 = new String(''); /*?+*/
    gamePiece = new GamePieces(this.Player1, this.Player2); /*?+*/
    ...
    * @param {String} _X, Player1's symbol, nominally Ex, X., or a Cross, Can be assigned new string type of piece/representation for the game. Optionally defaults to X.
    * @param {String} _O, Player2's symbol, nominally Zero, O, or a Nought. Can be assigned new string type of piece/representation for the game. Optionally defaults to O.
    * @constructor
    * @constructs Game
    * @type {Game}
    * @memberof Game
    ...
    */
     ...
     * @memberof Game
     * @param {*} move
     * @description Takes a move as an (array's) index to the board array and assigns the symbol to the move. For the terminal states of game (Win || Draw):
    * @todo  Either reset the board or do more on the UI
     */
    makeMove(move) {
        /** @name isRunning @type {Boolean} Takes the game start state*/
        let isRunning = this.gameStart
        // Check if gameLogic for win draw or win draw.
        /** @name move @type {number} */
        if (this.gameBoard.grid[move] === '' && !this.gameLogic.checkWin()) {
            // Assign current symbol  to the current move
            this.gameBoard.grid[move] = this.gamePieces.currentSymbol
