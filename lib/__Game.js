...
class Game {
    ...
    onInit(isRunning = true, pointerEvent, log = this.logLevel, locname = `${this.fileName}: onInit`) {
        // 1: Sets Block vars
        const logs = log;
        const check = this.errorLevel;
        const NOSTART = false;
        const initrunmessage = `The game is started started.`;
        const START = true;
        const initnorunmessage = `The game is not started. Click the start button again. This is not to be seen if isRunning True`;
        ...
        try {
            ...
            this.currentPiece = this.gamePieces.currentPiece;
            ...
            if (isRunning === this.RUNSTATE && isRunning === START) {
                console.log(initrunmessage);
                //this.gameRunning = this.isGameStarted(isRunning);
                this.gameRunning = isRunning;
                // this.gameRunning = this.RUNSTATE;
                const debugmessage = `Positive Flow: Game state is Running: ${isRunning}`;
                const debugtrace = `${locname}: ${isRunning}`;
                this.deBug._debug(isRunning, debugmessage, logs, debugtrace);
                //return this.gameRunning
            }
            if (isRunning !== this.RUNSTATE && isRunning === NOSTART) {
                // Should never reach here, just resets the game's running state to initial // Optional
                window.alert(initnorunmessage);
                return (this.gameRunning = this.INITSTATE);
            }
        } catch (errorinit) {
            const errormessage = `${locname}: Game Initialiser ${errorinit.linenumber}:${errorinit.columnNumber}`;
            const errortrace = `${locname}: Game Initialiser`;
            this.deBug._debug(errorinit, errormessage, check, errortrace);
        }
    }
     ...
     * @param {boolean} [isRunning] Toggles initial state of the game from initial state or User message @default this.INITSTATE
     ...
     */
    isGameStarted(isRunning = this.INITSTATE) {
    ...
    checkMove(move, VALID = true, log = this.logLevel, locname = `${this.fileName}: checkMove()`) {
        ...
        if (typeof move === TYPE) {
            ...
            return this.gameBoard.grid[cellId] === BLANK && this.gameRunning === this.RUNSTATE;
        }
        // 3: Game transitory inactive state (and or check the cell content??) or in moveListener.onClick
        if (VALID === INACTIVE && this.gameRunning === this.STOPSTATE) {
