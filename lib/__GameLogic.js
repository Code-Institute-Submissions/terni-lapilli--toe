 ...
 **/
  ...
  evaluateType(param, argIndex = 0) {
    ...
    if (!(param instanceof ( GameBoard || GamePieces))) {
      const paramName = paramNameMap[argIndex] || paramNameMap.default
      this.debug._debug(param,`GameLogic.js: Types: ${paramName} ${param} is ${param ? "inorrect type" : "unknow exception"}.`,10,`GameLogic.js`)
      throw new TypeError(`GameLogic.js: Types: ${paramName} ${param} is ${param ? "inorrect type" : "unknow exception"}.`)
    }
  }
  /**
   * @function gameLogger
