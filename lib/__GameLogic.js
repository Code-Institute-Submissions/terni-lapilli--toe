...
class GameLogic {
  constructor(_board,_pieces) {
    ...
    if (!_board || !_pieces) {
      throw new Error(
        `GameLogic: ${(_board || _pieces)} are required parameters.`,
    ...
    if (!(_board instanceof GameBoard)) {
      throw new TypeError(
        `GameLogic: ${_board} must be of type GameBoard`,
        {cause: ( _board )});
    }
    if (!(_pieces instanceof GamePieces)) {
      throw new TypeError(
        `GameLogic: ${_pieces} must be of type GamePieces.`,
        {cause: ( _pieces )});
    }
  ...
  isAWin() {
    ...
    **/
    const isWinningCombo = combination => {
      ...
      return this.board.surface[a] !== this.BLANK
        && this.board.surface[a] === this.board.surface[b]
        && this.board.surface[b] === this.board.surface[c];
