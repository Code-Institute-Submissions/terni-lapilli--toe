...
class GamePieces {
    ...
    setX(piece = "X") {
        ...
        try {
            if (this.checkAPiece(firstPiece, "string")) {
            ...
            throw new Error(errmessage, { name: `${locname}: Set X Error:`, stack: err });
        }
    }
    ...
    setO(piece = "O") {
        ...
        if (this.checkAPiece(secondPiece, "string")) {
            ...
            this.deBug._debug(currentPiece, errmessage, check, `${locname}: currentPiece`);
            throw new Error(errmessage, { name: `${locname}: Set O Error:`, stack: err });
        }
    }
    ...
    evaluatePieces(
        ...
        check = 5,
        type = "string",
        locname = `${this.fileName}: evaluatePieces()`
    ) {
        ...
        this.deBug._debug(symbol1, message, (log = 9), `${locname}`);
        try {
            if ((symbol1 ?? false) && (symbol2 ?? false)) {
                if (typeof symbol1 === literaltype && typeof symbol2 === literaltype) {
                    ...
                    // this.deBug._debug(symbol1, message, log, `${locname}`)
                    // return Array of strings
                    // (this.pieces[0] = symbol1) && (this.pieces[1] = symbol2)
                }
            }
        } catch (err) {
