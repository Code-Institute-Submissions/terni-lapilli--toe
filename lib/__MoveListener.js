...
class MoveListener {
    ...
    setListeners(nodes, game, evnt, log = this.logLevel, locname = `${this.fileName}: setListener()`) {
        nodes.forEach((cell) => {
            cell.addEventListener(evnt, this.onClick.bind(this, cell, game));
            this.deBug._debug(
                evnt,
                `Event ${evnt} clicked and cell and game bounded to the onClick`,
                `${locname}: onClick Bind`
            );
        });
    }
     ...
     * @date 2023/03/26
     * @date 2023/03/20
     * @version 0.1.0 => 0.4.0  FREEZE @date 2023/03/26.
            // this.deBug.debug(moveOutcome, `${moveOutcome}: Outcome`, `${locname}: makeMove() `);
     */
    onClick(cell, game, locname = `${this.fileName}: onClick`) {
        if (!cell.textContent) {
            const moveOutcome = game.makeMove(parseInt(cell.dataset.index), cell, `${locname}: makeMove`);

            if (moveOutcome) {
                cell.textContent = `${moveOutcome} X`;
                return;
            }
        }
