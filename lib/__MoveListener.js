...
class MoveListener {
    ...
    constructor(
        ...
        this.userMoves = document.querySelectorAll(moveID);
    ...
    setListeners(nodes, game, event, log = this.logLevel, locname = `${this.fileName}: setListener()`) {
        const logs = log;
        nodes.forEach((cell) => {
            cell.addEventListener(event, this.onClick.bind(this, cell, game, nodes, logs, locname));
            //const setlistentrace = `${locname}: onClick binds to ${cell}`;
            //const setlistenmessage = `Event ${event} clicked and cell and game bounded to the onClick`;
            //this.deBug._debug(event, setlistenmessage, logs, setlistentrace);
        });
    }
    ...
    onClick(cell, game, nodes, log = this.logLevel, locname = `${this.fileName}: onClick`) {
        ...
        this.deBug._debug(cell, onclickmessage, logs, onclicktrace);
        ...
        /** @type {boolean} RUN Game*/
        const ACTIVE = game.RUNSTATE;
        /** @type {boolean} STOP Game*/
        const INACTIVE = game.STOPSTATE;
        // 2.2 Game Terminus State
        /** @type {boolean} CONTINUE Playing:No Outcome */
        const CONTINUE = game.CONTSTATE;
        /** @type {boolean} END Playing: With Outcome */
        const TERMINATE = game.TERMINUS;
        const NOTNULL = false;
        const ISNULL = true;

        /** TODO REMOVE */
        debugger;
        let outcome = game.makeMove(moveId, cell, game.gameRunning, logs, `${locname}: makeMove`);
        /*?+*/
        //
        const debugmsg = `Parse of game.makeMove is ${outcome} === True`;
        const debugtrace = `${locname}: Check outcome`;
        const updatetrace = `${locname}: updateOnMove`;
        const endtrace = `${locname}: updateAtEnd`;
        debugger;
        this.deBug._debug(outcome, debugmsg, logs, debugtrace);
        /** TODO REMOVE */

        debugger; /** TODO REMOVE */
        // onClick Action if Cell is free, Game is active, and Terminal state is false
        if (game.gameRunning === ACTIVE && outcome === CONTINUE) {
            this.updateOnMove(cell, game, outcome, logs, updatetrace);
            return;
        }

        debugger; /** TODO REMOVE */
        // No Game active and Terminal state true: collectively clear/end game
        if (game.gameRunning === INACTIVE && outcome === TERMINATE) {
            this.updateAtEnd(nodes, game, outcome, logs, endtrace);
            return;
        }

        debugger; /** TODO REMOVE */
        if (outcome ?? ISNULL) {
            console.log("Is Undefined, not reaching UI condtrol");
            ...
            // Throw an error, deBUg to console and prompt user
            return;
        }
    }
