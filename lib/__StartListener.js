...
class StartListener {
     ...
     * @version 0.3.0 INSPECT @date 2023/03/22. See Changelog on this date.
     */
    constructor(button, game, evnt, locname = `${this.fileName} Constructor`) {
        ...
        this.newGame.whereAmICalled = `(${locname} || ${this.fileName}: exp Constructore)`;
        try {
            // Attatch Listener to Start Button
            this.addListener(this.startbutton, 1, 9);
        } catch (e) {
     ...
     * @version 0.3.0 INSPECT @date 2023/03/22. See Changelog on this date.
     */
    addListener(element, log = 0, check = 0, listerr = 3, locname = `${this.fileName}: addListener()`) {
        try {
            if (element ?? false) {
                ...
                this.newGame.whereAmICalled = `${locname} exp addListener}`;
    ...
    /** onStart [ ] TODO INSPECT Issue 4: check
     * @name onStart
     * @kind function
     * @type {void}
     ...
     * @version 0.3.0 INSPECT @date 2023/03/22. See Changelog on this date.
     */
    onStart(typerr = 2, log = 0, check = 0, locname = `${this.fileName}: onStart()`) {
        try {
            let message = `Starts a new game with`;

            if (this.newGame instanceof Game) {
                this.newGame.whereAmICalled = `${locname} Class Prop newGame}`;
                this.newGame = new Game("X", "Y");
                this.newGame.whereAmICalled = `${locname} new Object newGame instance}`;
                this.deBug._debug(this.newGame, message, log, `${locname}`);
            }
        } catch (e) {
     ...
     */
    onError(e, flag = 0, log = 9, locname = `${this.fileName}: onError()`) {
        const errorMessage = (() => {
            switch (flag) {
     ...
     * @version 0.3.0  FREEZE @date 2023/03/20. See Changelog on this date.
     */
    evaluateEventType(event, argIndex = 0, log = 0, locname = `${this.fileName}: evaluateEventType()`) {
