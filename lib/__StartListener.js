...
class StartListener {
     ...
     */
    constructor(game, btnId = "#start", evnt = "click", log = 0, locname = `${this.fileName} Constructor`) {
        //Santitise the @param game and THEN assign it to the member field.
        const aGame = this.evaluateParameter(game, 2, 10);
        this.newGame = aGame;
        //Santitise the @param button ident and before qualifying the selector query of Element during asssigment to the member field.
        ...
        try {
            ...
            this.deBug._debug(this, `${this}: addListener`, log, `${locname}: addListener`);
        } catch (e) {
            this.onError(e, 5, 9, `${locname}`);
        }
    }

    onInit(game, log = 0, locname = `${this.fileName}: onInit()`) {
        try {
            // Eval a 2nd time, the game param, OVERKILL??
            const _game = this.evaluateParameter(game, 2, 10);
            // Log to console
            this.deBug._debug(_game, message, log, `${locname}`);
     ...
     * @version 0.3.0 INSPECT @date 2023/03/22. See Changelog on this date.
     */
    addListener(element, game, log = 0, check = 0, errlistener = 3, locname = `${this.fileName}: addListener()`) {
        try {
     ...
     */
    onError(e, errflag = 0, log = 9, locname = `${this.fileName}: onError()`) {
        const errorMessage = (() => {
            switch (errflag) {
     ...
     * @version 0.3.0  FREEZE @date 2023/03/20. See Changelog on this date.
     */
    evaluateEventType(event, eventIndex = 0, log = 0, locname = `${this.fileName}: evaluateEventType()`) {
