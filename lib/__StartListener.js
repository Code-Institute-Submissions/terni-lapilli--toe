...
class StartListener {
  ...
  constructor(_button,_game,_evt) {
    try {
      ...
      /**  @prop {NodeList} - Button object from the App class's instance */
      this.button = _button
      /** @prop {Game} - Game object from the App class's instance */
      this.game = _game
      /** @prop {String}  - Event: User Clicks events */
      this.eventType = _evt
      /** @prop {GameConfig} - GameConfig fpr the current games configuration strings */
      this.config = _game.gameConfig
      ...
      if (!(_button instanceof NodeList)) {
        //Debugger
        this.debug._debug(_button,`Button: ${_button} is not defined at runtime.`,6)
        this.error(_button,3)
      }
      // Defensive check for Constructor param types
      if (!(_game instanceof Game)) {
        //Debugger
        this.debug._debug(_game,`Game: ${_game} is not defined at runtime.`,6)
        this.error(_game,3)
      }
  ...
  addListener() {
    ...
    try {
        if (this.button) {
            ...
            this.debug._debug(this.button,`Button: ${this.button} is present`,9)
      ...
      this.debug._debug(this.button,`${e.name} : ${this.button} is not defined`,9)
      this.error(e,1)
    }
 }
  ...
  onStart() {
    ...
    try {
        this.game = new Game('X','Y',(new GameConfig()))
  ...
  error(e,flag) {
    switch (flag) {
      case 1:
        this.debug._debug(e,`Error: ${e.name} is not defined!`,6)
        throw new Error(
      ...
      case 2:
        this.debug._debug(e,`Error: Instance of: ${e.name}: ${e.toString()} is not instantiated`,9)
        throw new Error(
      ...
      case 3:
        this.debug._debug(e,`Type: Instance of: ${e.name}: ${e.toString()} is not correct`,9)
        throw new TypeError(
      ...
      case 4:
        this.debug._debug(e,`Error: One of Button | Game | Event: ${e.name}: ${e.toString()} is not provided | at runtime`,9)
        throw new Error(
      ...
      default:
        this.debug._debug(e,`Error: ${e.name} Default: ${e.toString()} is not being handled`,3)
        break;
    }
  }
}
