 ...
 * @date 2023/03/09
 * @date 2023/03/19 @version 0.3.0
 * @date 2023/03/24 @version 0.4.0  @see Issue#4 @link https://github.com/iPoetDev/terni-lapilli--toe/issues/4
 *  @date 2023/03/31 @version 0.4.0 @see Issue#12 @link https://github.com/iPoetDev/terni-lapilli--toe/issues/12
 */
class StartListener {
    ...
    logLevel = 1; // Logger Output cleared
     ...
     * @param {number} locname Trace method name @default this.fileName
     ...
     *  @version 0.4.1 @date 2023/03/31 @see Issue#12
     */
    constructor(game, btnID = "#start", evnt = "click", log = this.logLevel, locname = `${this.fileName} Constructor`) {
        ...
        try {
            ...
            this.addListener(this.startButton, this.newGame, logs, `${locname}: addListener, new Game`);
            this.deBug._debug(this.startButton, `${this}: addListener`, logs, `${locname}: addListener, new Game`);
        } catch (e) {
            const errorflag = 1;
            this.onError(e, errorflag, check, `${locname}`);
        }
    }
    ...
    onInit(game, log = this.logLevel, locname = `${this.fileName}: onInit()`) {
        ...
        try {
            ...
            if (!_game.gameRunning) {
                const message = `const: game ${_game} && truthy is ${_game.gameRunning}`;
                // Initialise the game and log to Trace (8) level
                this.deBug._debug(_game, message, logs, `${locname}: Game is initialised`);
                // Initialise the game and log to Trace (5) level
                const isInit = _game.isGameStarted(false);
                _game.onInit(isInit, logs);
            }
        } catch (e) {
            const errorflag = 1;
            this.onError(e, errorflag, check, `${locname}: Initialisation Error `);
        }
    }
     ...
     * @usage TBC
     * @returns {boolean} Whether the start button is pressed and the game has been started
     * @memberof Game
     ...
     * @date 2023/03/31.@version 0.4.1 @see Issue#12
     */
    addListener(
        element,
        game,
    ...
    onError(error, errflag = 0, log = this.errorLevel, locname = `${this.fileName}: onError()`) {
        ...
        switch (errflag) {
            case 1: // General Error:  Null | Not Defined
                this.deBug._debug(error, errorMessage[errflag], check, `${locname}`);
            ...
            case 3: // EventListener Binding Errors
                this.deBug._debug(error, errorMessage[errflag], check, `${locname}: addListener Binding`);
                throw new Error(errorMessage, { name: `${locname} Binding Error:`, stack: error });
            default: // Uncatched Errors
                this.deBug._debug(error, errorMessage.default, check, `${locname}`);
                break;
        }
    }
