...
class StartListener {
    ...
    constructor(game, btnID = "#start", evnt = "click", log = 0, filename = `StartListener.js`) {
        ...
        try {
            // @ts-ignore
            const buttonmessage = `${this.startButton}: addListener`;
            const buttontrace = `${locname}: addListener, new button, new Game`;
            //@ts-ignore
            // 5: Attatch Listener to Start Button
            this.addListener(this.startButton, this.newGame, logs, buttontrace);
            this.deBug._debug(this.startButton, buttonmessage, logs, buttontrace);
        } catch (error) {
            const errorflag = 1;
    ...
    /** onInit() @version 0.4.2 @date 2023/04/02
     * @function onInit()
     * @kind function
    ...
    onInit(game, log = this.logLevel, locname = `${this.fileName}: onInit()`) {
        ...
        const logs = 8;
        const check = this.errorLevel;
        const debugparam = `Param: game ${game}`;
        const gamemessage = `${locname}: Game Running`;
        const debugtrace = `${locname}: Game is initialised`;
        const START = true;
        const NOSTART = false;

        // 2: Error handling on Initialisation
        try {
            //this.deBug._debug(game, debugparam, logs, debugtrace);
            // 3: Eval a 2nd time, the game param, OVERKILL??
            //game = this.evaluateParameter(game, 2, log, debugtrace);
            //this.newGame = game;
            // 4: Check if game is falsey, which is set when a game is instatiated,
            // @ts-ignore
            if (game.gameRunning === NOSTART) {
                this.deBug._debug(game, gamemessage, logs, debugtrace);
                // @ts-ignore
                //const isInit = this.newGame.isGameStarted(NOSTART);
                // Added: 04/02 3pm
                //this.isStartClicked();
                // @ts-ignore
                this.newGame.onInit(START, null, logs, debugtrace);
            }

            if (game.gameRunning === START) {
                const usermessage = `This message just says you pressed Start during a game. Carry on playing`;
                window.alert(usermessage);
            }
        } catch (error) {
            const errorflag = 1;
            const errortrace = `${locname}: Initialisation Error ${error.linenums}`;
            this.onError(error, errorflag, check, errortrace);
        }
    }

    /** isStartClicked @version 0.4.2 @date 2023/04/02
     ...
     * @usage TBC
     * @constant START @default true
     * @constant NOSTART @default false
    ...
    isStartClicked() {
        const NOSTART = false;
        const START = true;
        if (this.startClicked === NOSTART) {
            // @ts-ignore
            const startmessage = `The game's start button is BEING pressed`;
            // @ts-ignore
            window.alert(startmessage);
            return (this.startClicked = true);
        }

        if (this.startClicked === START) {
            // @ts-ignore
            const clickmessage = `The game's start button has been pressed. Game Started`;
            // @ts-ignore
    ...
    addListener(element, game, log = this.logLevel, locname = `${this.fileName}: addListener()`) {
        ...
        const NOTNULL = false;
        try {
            if ((element ?? NOTNULL) || (game ?? NOTNULL)) {
                //Message string
